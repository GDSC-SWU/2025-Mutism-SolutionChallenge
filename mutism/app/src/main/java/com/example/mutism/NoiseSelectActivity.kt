...
class NoiseSelectActivity :
    ...
    override fun onTagDeselected(tag: String) {
        selectedTags.remove(tag)
        for (i in 0 until selectedTagChipGroup.childCount) {
            val chip = selectedTagChipGroup.getChildAt(i) as Chip
            if (chip.text == tag) {
                selectedTagChipGroup.removeViewAt(i)
                break
            }
        }
    }
